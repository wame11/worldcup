...
async function claimCode(code, name) {
  const ref = doc(db, "codes", code);
  await setDoc(ref, { claimed: true, name, claimedAt: serverTimestamp() }, { merge: true });
}

async function fetchPredictions(code) {
  const ref = doc(db, "predictions", code);
  const snap = await getDoc(ref);
  return snap.exists() ? snap.data() : null;
}
...
async function fetchResults() {
  const snap = await getDoc(doc(db, "results", "global"));
  return snap.exists() ? snap.data() : { groups: {}, bracket: {}, champion: null, third: null };
}

async function saveResults(payload) {
  await setDoc(doc(db, "results", "global"), { ...payload, updatedAt: serverTimestamp() }, { merge: true });
}

async function fetchAllPredictions() {
  const snap = await getDocs(collection(db, "predictions"));
  const out = [];
  snap.forEach((d) => out.push({ code: d.id, ...d.data() }));
  return out;
}
...
function renderMatchRow(m) {
  ...
  const outcomeBtn = (val, label) => `
    <button type="button" class="outcome-btn ${winner === val ? "is-selected" : ""}" data-pick="${val.toLowerCase()}">${label}</button>`;

  return `
    <div class="match-row" data-match-id="${m.id}">
      <div class="match-row__num">${String(m.id).padStart(2, "0")}</div>
      <div class="match-row__date">${formatDate(m.date)} · ${m.time}</div>
      ${teamChip(m.home, { side: "home" })}
      <div class="match-row__pick">
        <input class="score-input" data-side="home" type="number" min="0" max="20" value="${sh}" inputmode="numeric" aria-label="${m.home} score" />
        <span class="score-dash">—</span>
        <input class="score-input" data-side="away" type="number" min="0" max="20" value="${sa}" inputmode="numeric" aria-label="${m.away} score" />
      </div>
...
function renderBracketTab() {
  ...
  const sections = BRACKET_ROUNDS.map((r) => bracketSection(r, b[r.id] || [])).join("");
  const finals = bracketFinals(b);
  root.innerHTML = sections + finals;
  wireBracket(root);
}

function bracketSection(round, picks) {
  return `
    <div class="bracket-section" data-round="${round.id}">
      ...
      <div class="team-picker">
        ${TEAM_CODES.map((code) => {
          ...
          const t = TEAMS[code];
          return `
            <button class="team-chip ${sel}" type="button" data-team="${code}">
...
function bracketFinals(b) {
  const opts = TEAM_CODES.map((c) =>
    `<option value="${c}">${TEAMS[c].name} (${c})</option>`).join("");
  return `
  ...
  $$(".bracket-section[data-round]", root).forEach((section) => {
    ...
    $$(".team-chip", section).forEach((chip) => {
      chip.addEventListener("click", () => {
        ...
        const picks = new Set(session.predictions.bracket[roundId] || []);
...
function scoreOnePerson(p, results) {
  ...
  for (const round of BRACKET_ROUNDS) {
    const picks   = bp[round.id]   || [];
    const actual  = br[round.id]   || [];
...
async function renderAdminResults() {
  ...
  const bracketRows = BRACKET_ROUNDS.map((round) => {
    const picks = results.bracket[round.id] || [];
    return `
  ...
  $$("[data-result-round-chip]").forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      const arr = new Set(adminResultsDirty.bracket[round] || results.bracket[round] || []);
...
async function seedCodes() {
  ...
  out.textContent += "Writing to Firestore…\n";
  const batch = writeBatch(db);
  list.forEach((code) => {
    batch.set(doc(db, "codes", code), { claimed: false, createdAt: serverTimestamp() }, { merge: true });
  });
  try {
    await batch.commit();
