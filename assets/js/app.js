...
import {
  getDatabase, ref, get, update, serverTimestamp,
...
async function fetchAllPredictions() {
  ...
  const out = [];
  snap.forEach((d) => out.push({ code: d.key, ...d.val() }));
  return out;
}
...
async function seedCodes() {
  ...
  const updates = {};
  list.forEach((code) => {
    updates[`codes/${code}/claimed`] = false;
    updates[`codes/${code}/createdAt`] = serverTimestamp();
  });
  try {
