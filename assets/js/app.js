...
function isMatchLocked(match) {
  return Date.now() >= kickoffDateTime(match).getTime();
}
...
function isBracketPredictionLocked() {
  ...
  return firstR32 && Date.now() >= firstR32.getTime();
}
...
function renderMatchRow(m) {
  ...
  return `
    <div class="match-row" data-match-id="${m.id}" data-locked="${locked}">
      ...
      <div class="match-row__date">${formatDate(m.date)} · ${m.time}${lockedLabel}</div>
...
async function renderBracketTab() {
  ...
  $("#bracket-save-status").textContent = bracketLocked ? "Locked after first Round of 32 kickoff" : "Saved automatically";
