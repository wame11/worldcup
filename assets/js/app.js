...
async function renderBracketTab() {
  ...
  $("#bracket-save-status").textContent = "Saved automatically";
  const sections = PLAYER_BRACKET_ROUNDS.map((r) => bracketSection(r, asList(b[r.id]), knockoutTeams)).join("");
  const finals = bracketFinals(b, knockoutTeams);
  root.innerHTML = sections + finals;
  wireBracket(root);
}

function bracketSection(round, picks, teamCodes = TEAM_CODES) {
  return `
...
function bracketFinals(b, teamCodes = TEAM_CODES) {
  const opts = teamCodes.map((c) =>
